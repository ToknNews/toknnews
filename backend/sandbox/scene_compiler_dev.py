...
# === ToknNews Semantic Recall (Sandbox) ===
import sqlite3, numpy as np
from sentence_transformers import SentenceTransformer

RECALL_DB = "/var/www/toknnews/devdata/memories.db"
...
def recall_context(query_text, top_k=3):
    """Fetch Chip’s most semantically similar past lines (sandbox)."""
...
def compile_scene(headline, source="manual", character="chip"):
    ...
    def generate_narrative_prompt(scene_dict):
        try:
            ...
            print(f"[Compiler] ⚠️ generate_narrative_prompt error: {e}")
...
def generate_narrative_prompt(scene_dict):
    try:
        ...
        # --- Sandbox-only output path ---
        sandbox_output = "/var/www/toknnews/devdata/latest_narrative.json"
        json.dump({
            "generated_at": scene_dict.get("time"),
            "topic": scene_dict.get("topic"),
            "narrative": narrative
        }, open(sandbox_output, "w"), indent=2)

        print(f"[Sandbox Broadcast] 🧩 Generated narrative for {scene_dict.get('topic','(unknown)')} → {sandbox_output}")
    except Exception as e:
        print("[Sandbox Broadcast] Narrative generation skipped:", e)
