...
def fetch_rss(url):
    try:
        ...
        print(f"[RSS] ⚠️  Failed to fetch {url}: {e}")
        return []        # skip this feed and move on
    items = []
...
if __name__ == "__main__":
    ...
    # === Auto-trigger ElevenLabs Exporter after successful run ===
    import subprocess
    try:
        subprocess.run(
            ["python3", "/var/www/toknnews/devmodules/elevenlabs_exporter.py"],
            check=True
        )
        print("[System] 🗣️ ElevenLabs exporter completed successfully.")
    except Exception as e:
        print(f"[System] ⚠️ ElevenLabs exporter failed: {e}")

    # === Log latest dialogue into SQLite memory (dev) ===
    try:
        import subprocess
        subprocess.run(
            ["python3", "/var/www/toknnews/devmodules/memory_logger.py"],
            check=True
        )
        print("[System] 🧠 Memory logger inserted latest lines into SQLite.")
    except Exception as e:
        print(f"[System] ⚠️ Memory logger failed: {e}")
