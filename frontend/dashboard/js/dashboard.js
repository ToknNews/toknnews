/* ==========================================================
   TOKNNEWS DASHBOARD — CLEAN RESTORE BUILD
   ========================================================== */
...
async function loadSystemSummary() {
  try {
    ...
    velEl.textContent =
      hb.total_compiles > 0
        ? `${hb.total_compiles} compiles • ${hb.runtime_seconds}s`
        : "--";

    // === FEEDS ===
    const srcEl = document.getElementById("sources");
    srcEl.textContent =
      (hb.active_sources && hb.active_sources > 0)
        ? `${hb.active_sources} active`
        : (hb.last_headline ? "Feeds updating" : "--");
...
async function renderChart(rangeDays = 7) {
  ...
  try {
    const res = await fetch("/data/daily_tally.json", { cache: "no-store" });
    const data = await res.json();
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    await loadSystemSummary();   // ← add this line
    await loadSummary();
    await renderChart(7);
    await loadScenes();
  } catch (err) {
